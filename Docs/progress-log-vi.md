...
bundle nào nên chưa bao giờ được nạp.

Trước khi chạy được, `websocket-client` còn thiếu trong venv → `start_tour`
**trả về mà không lái trình duyệt**, test báo xanh trong khi chưa kiểm gì. Đây
là điểm cần nhớ: một bộ tour không với tới được trình duyệt thì vẫn xanh.

Tour thật tìm ra ngay hai lỗi mà **không test model nào bắt được**:

| Lỗi | Vì sao test model không thấy |
|---|---|
| `view_mode` còn xin `tree` ở 5 action | là **nội dung phần tử**, cổng của tôi chỉ biết dạng thuộc tính |
| kanban dùng `t-name="kanban-box"` | Odoo 19 đổi thành `card`; **không lỗi lúc cài** — view nạp bình thường rồi trình duyệt ném "Missing 'card' template." khi có người mở |

Cả hai đã sửa và đã thêm vào `tools/tests/test_view_syntax.py`.
...
## 2026-08-20 - Menu theo vong van hanh + bo bao cao quan ly (aic_okr_kpi)

Yeu cau: menu chua logic; khach thieu bao cao tien do theo thoi gian, theo
phong ban, theo vi tri.

**Menu.** 17 muc phang duoi mot app, ke hoach lan voi thuc hien va danh gia,
va nhom Configuration chua 18 muc trong do 5 muc la thao tac (Import
Spreadsheet, Import Actuals, Roll Over Cycle, Apply Library Pack, Capture
Knowledge). Mot lan nhap so lieu hang thang khong nen nam canh nguong RAG.
Gio gom thanh 7 nhom theo dung vong PDCA ma san pham dang ban. 26 muc chuyen
bang bang anh xa chu khong sua tay. Nguyen tac phan loai ghi vao comment cua
file menu.

**Hai lo hong dieu huong.** `aic.hrm.kpi.period.result` - noi chua so lieu
thang/quy - khong co action lan menu, chi toi duoc qua form KPI target. Va hai
muc Configuration cung sequence 5 nen thu tu do database quyet dinh.

**Bao cao.** Toan suite truoc do khong co mot view graph hay pivot nao. Model
moi `aic.hrm.progress.report` la SQL view union check-in voi period result da
xac nhan; graph, pivot, list va dashboard Executive Overview cung doc mot
model nen khong the lech so.

**Bon quyet dinh ky thuat** (chi tiet trong PROGRESS.md): ky vong tinh bang SQL
tai ngay do luong; id suy tu dong nguon thay vi row_number(); view tu flush
model nguon; cot vai tro thu vien do module thu vien dong gop qua hook.

**Kiem chung tren du lieu that** (may chu demo khach, cong 8074): 188 dong =
56 check-in + 132 ket qua ky, dung bang so nguon. Nhom theo thang, theo phong
